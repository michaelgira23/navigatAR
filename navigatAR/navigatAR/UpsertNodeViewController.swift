		...
		ref.observeSingleEvent(of: .value, with: { snapshot in
			...
			let tags: [String: Tag] = self.tagInfos.reduce(into: [:]) { (result, tagInfo) in
				if let formValue = formValues[tagInfo.name]! {
					...
					case (.string, true):
						tagValue = Tag.multipleStrings(FirebaseArray(values: (formValue as! [String?]).flatMap { $0 }))
					case (.number, true):
						tagValue = Tag.multipleNumbers(FirebaseArray(values: (formValue as! [Int?]).flatMap { $0 }))
					default:
